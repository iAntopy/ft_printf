...
/*   Updated: 2022/03/31 22:22:06 by iamongeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	rec_puthex(size_t ptr, char *base)
{
	if (ptr >= 16)
	{
		rec_puthex(ptr / 16, base);
		g_char_count++;
	}
...
{
	g_char_count = 1;
	rec_puthex(nbr, HEX_BASE_UPPERCASE);
	return (g_char_count);
}
...
{
	g_char_count = 1;
	rec_puthex(nbr, HEX_BASE_LOWERCASE);
	return (g_char_count);
}
...
{
	g_char_count = 1;
	rec_puthex(ptr, HEX_BASE_LOWERCASE);
	return (g_char_count);
}
